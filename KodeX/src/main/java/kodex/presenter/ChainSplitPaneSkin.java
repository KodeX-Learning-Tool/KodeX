...
public class ChainSplitPaneSkin extends SkinBase<ChainSplitPane> {
   ...
   * {@link Control#getChildren() children} list, as well as the necessary input mappings for
    ...
    for (int trys = 0; trys < 10; trys++) {
          ...
          space =
              (horizontal ? w : h)
                  - (previousDivider != null
                      ? getAbsoluteDividerPos(previousDivider) + dividerWidth
                      : 0);
        }
      ...
      for (Content c : contentRegions) {
        ...
        if (resize && !c.isResizableWithParent()) {
          // We are making the ChainSplitPane bigger and will need to
          // distribute the extra space.
          if (c.getArea() >= c.getResizableWithParentArea()) {
            extraSpace += (c.getArea() - c.getResizableWithParentArea());
          } else {
            // We are making the ChainSplitPane smaller and will need to
  ...
  private void addDivider(ChainSplitPane.Divider d) {
  ...
  private void initializeDivderEventHandlers(final ContentDivider divider) {
    ...
    divider.addEventHandler(MouseEvent.ANY, MouseEvent::consume);
    ...
    divider.setOnMouseDragged(
        e -> {
           ...
           * when they can't move because of a neighbor that blocks them,
  ...
  private void setAbsoluteDividerPos(ContentDivider divider, double value) {
    if (getSkinnable().getWidth() > 0 && getSkinnable().getHeight() > 0 && divider != null) {
      ChainSplitPane.Divider paneDivider = divider.getDivider();
  ...
  // Updates the divider with the ChainSplitPane.Divider's position
  // The value updated to ChainSplitPane.Divider will be the center of the divider.
  ...
  // Pos is the percentage location from ChainSplitPane.Divider.
  ...
  private double totalMinSize() {
    double dividerWidth =
        !contentDividers.isEmpty()
            ? contentDividers.size() * contentDividers.get(0).prefWidth(-1)
            : 0;
    double minSize = 0;
    for (Content c : contentRegions) {
      if (horizontal) {
  ...
  private void layoutDividersAndContent(double width, double height) {
    ...
    final double dividerWidth =
        contentDividers.isEmpty() ? 0 : contentDividers.get(0).prefWidth(-1);

    for (Content c : contentRegions) {
      //          System.out.println("LAYOUT " + c.getId() + " PANELS X " + c.getX() + " Y " +
      // c.getY() + " W " + (horizontal ? c.getArea() : width) + " H " + (horizontal ? height :
      // c.getArea()));
      if (horizontal) {
    ...
    for (ContentDivider c : contentDividers) {
      //          System.out.println("LAYOUT DIVIDERS X " + c.getX() + " Y " + c.getY() + " W " +
      // (horizontal ? dividerWidth : width) + " H " + (horizontal ? height : dividerWidth));
      if (horizontal) {
  ...
  class ContentDivider extends StackPane {
    ...
    public ContentDivider(ChainSplitPane.Divider d) {
      ...
      // TODO register a listener for ChainSplitPane.Divider position
    }
  ...
  static class Content extends StackPane {
    ...
    public boolean isResizableWithParent() {
      return ChainSplitPane.isResizableWithParent(content);
    }
